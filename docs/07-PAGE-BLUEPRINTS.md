...
| 8 | **Spa preview** *(SHIPPED — `components/home/SpaPreview.tsx`)* | the page's **quiet exhale** — stillness, restoration, privacy, escape | the most intimate composition, distinct from §5/§6/§7: a warm, dim, **candlelit** ground (`bg-[#1b140c]` + `.spa-ground`), ONE soft-focus sensory placeholder (`.spa-*`, shallow-DoF/tactile, honestly labelled *"Spa photography required …"*), restrained copy floating in generous negative space, restrained reveal. **NAME-NEUTRAL** — never "Allura"; **emotional role only**, no invented services/treatments/pricing/hours/brands/facilities/packages. | **dark warm, asymmetric, minimal** | **Explore the Spa** → /spa (`explore_spa`) · **Enquire** → /contact (`spa_enquire`) |
| 9 | **Dining preview** *(SHIPPED — `components/home/DiningPreview.tsx`)* | the **image-FIRST** section — the photograph does the emotional work, copy supports | a new composition distinct from §5–§8: a **full-bleed** dining image band (edge-to-edge `.dine-*` placeholder, warm/appetising, subtle set-table glints) with a **restrained overlaid copy panel** on a scrim. Sells the **experience** (gathering · shared meals · conversation · hospitality) + a **certified-halal** chip (BKB-confirmed). **NAME-NEUTRAL** — no restaurant name; **NO** invented menu/cuisine/pricing/chefs/hours; **NO alcohol/bar** (docs/17 §3.6). Photography honestly labelled *"Dining photography required …"*. | **full-bleed image + overlaid copy** | **Explore Dining** → /dining (`explore_dining`) · **Enquire** → /contact (`dining_enquire`) |
| 10 | **Gallery showcase** *(SHIPPED — `components/home/GalleryShowcase.tsx`)* | the homepage's **emotional peak** — breadth of the estate; invites further exploration | **NOT a grid**: an editorial **mosaic** (a signature feature + a tall accent, then a trio) with slow motion-safe hover-zoom. Image-FIRST, copy minimal. Tiles reuse the labelled placeholder hues (glass hall · weddings · conferences · gardens · dining) — **every tile a clear placeholder** until the curated gallery is shot (docs/17 §3.10). **Light** ground = deliberate shift after the two dark sections (Spa, Dining). | **light, editorial mosaic** | **Explore the gallery** → /gallery (`explore_gallery`) |
| 11 | **Reviews / Trust** *(SHIPPED — `components/home/ReviewsTrust.tsx`)* | the turn from emotion → conversion; answers *"can I trust them with the most important occasion?"* | **NOT** a slider/cards/widget/wall of text — quiet & **editorial**: a confident serif statement + two restrained typographic pull-quotes (champagne quote-marks, hairline divider) + an "awaiting" aggregate. Progressively quieter (more confidence, more breathing room). **Review count / testimonials / grading are Gate-1 + consent** → the aggregate & quotes are **clearly-temporary placeholders**, architected with `data-google-reviews` / `data-review-slot` to populate from **live Google reviews** later. **NO `AggregateRating` schema until consented.** | **light `mist`, spare, centered** | (no CTA — the Final CTA §14 follows) |
