...
| — | *Shipped homepage sequence (current):* Hero → Journey selector → Estate Introduction → **Conferences preview (§6)** → **Weddings preview (§5)**. Weddings currently follows Conferences on the page (capability → emotion turn); final §5-before-§6 ordering is a homepage-composition decision to confirm at review. |
