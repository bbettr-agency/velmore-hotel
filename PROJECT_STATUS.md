...
- ➡️ **Next (on approval):** resume the **homepage — Weddings Preview** (per docs/07 §5), then remaining sections (Stay/Spa/Dining/Events/gallery/trust/final-CTA/footer), one reviewable milestone at a time. Then, once the homepage is fully approved, pages in order: Conferences → Weddings → Accommodation → Dining → Spa → Events → Gallery → About → Contact.
