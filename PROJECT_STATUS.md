...
- 📌 **STANDING RULE — photography-first process:** every section = image → composition → copy → CTA (images create emotion · copy provides context · CTAs capture intent). Never let text compete with photography; each section a distinct feeling+composition.
- ➡️ **Next (on approval):** continue the **homepage** — reviews/trust, final-CTA, footer (per docs/07), one reviewable milestone at a time. Then, once the homepage is fully approved, pages in order: Conferences → Weddings → Accommodation → Dining → Spa → Events → Gallery → About → Contact.
