...
- ✅ **Confirmed homepage order:** Hero → JourneySelector → EstateIntro → **Conferences → Weddings → Stay → Spa → Dining**. Section-feeling rhythm (client-endorsed): anticipation · confidence · capability · emotion · calm · restoration · gathering.
- ➡️ **Next (on approval):** continue the **homepage** — gallery / trust / final-CTA / footer (per docs/07), one reviewable milestone at a time. **Standing: every remaining section is designed image-first.** Then, once the homepage is fully approved, pages in order: Conferences → Weddings → Accommodation → Dining → Spa → Events → Gallery → About → Contact.
