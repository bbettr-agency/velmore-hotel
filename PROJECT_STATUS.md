...
- 🔒 **Milestone 1 (first viewport hero) LOCKED** — untouched.
...
- ➡️ **Next (on approval):** continue the **homepage, section by section** — next section = **estate introduction + capability/scale proof** (docs/07 homepage order §3–4). Then, once the homepage is fully approved, pages in order: Conferences → Weddings → Accommodation → Dining → Spa → Events → Gallery → About → Contact.
