...
- ✅ **Confirmed homepage order:** Hero → JourneySelector → EstateIntro → **Conferences → Weddings → Stay** (capability → emotion → rest; Conferences-first is the client-approved commercial order).
- ➡️ **Next (on approval):** continue the **homepage** — Spa / Dining previews, then trust / final-CTA / footer (per docs/07), one reviewable milestone at a time. Then, once the homepage is fully approved, pages in order: Conferences → Weddings → Accommodation → Dining → Spa → Events → Gallery → About → Contact.
