...
- ✅ **Confirmed homepage order:** Hero → JourneySelector → EstateIntro → **Conferences → Weddings → Stay → Spa** (capability → emotion → rest → exhale; Conferences-first is the client-approved commercial order).
- ➡️ **Next (on approval):** continue the **homepage** — Dining preview, then trust / gallery / final-CTA / footer (per docs/07), one reviewable milestone at a time. Then, once the homepage is fully approved, pages in order: Conferences → Weddings → Accommodation → Dining → Spa → Events → Gallery → About → Contact.
