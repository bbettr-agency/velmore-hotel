...
- 💍 **Milestone 5 SHIPPED & LOCKED — Homepage Weddings Preview** (`components/home/WeddingsPreview.tsx`): emotive counterpoint to Conferences (copy-left / portrait image-right / tinted `mist` / italic kicker / hairline "Held for you"). Confirmed facts only; `.wed-*` labelled placeholder.
- 🛏️ **Milestone 6 SHIPPED — Homepage Stay Preview** (`components/home/StayPreview.tsx`): the calm/rest change of pace ("everything is here — no need to leave"). Centered, spacious, image-led (NOT a split) — wide environmental + garden detail placeholders (light/airy `.stay-*`, honestly labelled "Photography required"), quiet serif proof trio, restrained reveal. Confirmed facts only (50 rooms · on the estate · gardens; no invented categories/rates; restoration rooms never shown). CTAs Explore Your Stay (/accommodation) + Check Availability (/contact — enquiry journey, distinct `check_availability` label for future booking engine).
- 🌿 **Milestone 7 SHIPPED — Homepage Spa Preview** (`components/home/SpaPreview.tsx`): the page's quiet exhale (stillness/restoration/privacy/escape). Warm, dim, candlelit, asymmetric, minimal — one soft-focus sensory placeholder (`.spa-*`) + restrained copy + generous negative space. **Name-neutral (never "Allura"); emotional role only** — no invented services/treatments/pricing/hours/brands/facilities/packages. CTAs Explore the Spa (/spa, `explore_spa`) + Enquire (/contact, `spa_enquire`).
- 🍽️ **Milestone 8 SHIPPED — Homepage Dining Preview** (`components/home/DiningPreview.tsx`): the **image-first** section (client directive — design around the image; photo carries the emotion, copy supports). Full-bleed dining band (`.dine-*`) + restrained overlaid copy + certified-halal chip. **Name-neutral; sells the experience** (gathering/shared meals/conversation/hospitality); no invented menu/cuisine/pricing/chefs/hours; no alcohol/bar. CTAs Explore Dining (/dining, `explore_dining`) + Enquire (/contact, `dining_enquire`).
- 🖼️ **Milestone 9 SHIPPED — Homepage Gallery Showcase** (`components/home/GalleryShowcase.tsx`): the emotional **peak**. Editorial **mosaic** (feature + tall accent + trio), NOT a grid; image-first, hover-zoom, light ground (shift after the dark Spa/Dining). Tiles reuse labelled placeholders (all marked "· placeholder"). CTA Explore the gallery (/gallery, `explore_gallery`).
- ✅ **Confirmed homepage order:** Hero → JourneySelector → EstateIntro → **Conferences → Weddings → Stay → Spa → Dining → Gallery**. Section-feeling rhythm: anticipation · confidence · capability · emotion · calm · restoration · gathering · **peak**.
- 🤝 **Milestone 10 SHIPPED — Homepage Reviews / Trust** (`components/home/ReviewsTrust.tsx`): the turn from emotion → conversion. Quiet, editorial, confident (NOT a slider/cards/widget) — a serif confidence statement + two typographic pull-quotes + an "awaiting" aggregate. Review data = clearly-temporary placeholders, architected (`data-google-reviews`/`data-review-slot`) to populate live Google reviews later; **no AggregateRating schema until consented**.
- 📌 **STANDING RULES:** (a) **photography-first** — every section = image → composition → copy → CTA; (b) **progressively-quieter close** — from the Gallery on, sections get quieter (less content, more confidence, more breathing room) as the page turns emotion → conversion.
- 🎬 **Milestone 11 SHIPPED — Homepage Final CTA** (`components/home/FinalCta.tsx`): the natural conclusion ("the next step is easy"). Image-first warm golden-hour full-bleed close (bookends the dusk hero), centered minimal copy, primary Book a viewing + secondary Enquire for your date + reassurance line. Not loud/salesy.
- 🗂️ **Production image library scaffolded** — `public/images/**` (22 `.gitkeep` folders) + `public/images/README.md` (conventions, formats, sizes, alt/SEO, per-folder photography status). No placeholder images; no code references yet; build+deploy verified.
- 🏁 **HOMEPAGE COMPLETE & LOCKED** — Footer (`components/site/Footer.tsx`) shipped; 11 sections + footer.
- 🏭 **PRODUCTION MODE** (client directive): build the whole site page-by-page, push continuously, verify prod, no per-section stops. Shared config-driven **section kit** (`components/sections/`: PageHero·FeatureSplit·Statement·SpecList·ImageBand·Faq·CtaBand·Placeholder) + `config/pages.ts`. Image-first; confirmed facts only; labelled placeholders.
- **Inner pages:** ✅ Conferences · ⏳ Weddings · Accommodation · Dining · Spa · Events · Gallery · About · Contact. Then full production QA. Then, once the homepage is fully approved, pages in order: Conferences → Weddings → Accommodation → Dining → Spa → Events → Gallery → About → Contact.
