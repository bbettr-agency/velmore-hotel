...
| 20 | **Name-neutral, service-neutral section pattern** (`SpaPreview`, `DiningPreview`) | when a sub-brand's name/operator/services are unconfirmed, ship a section that communicates only the *emotional role/experience* + an enquiry CTA, asserting nothing unconfirmed — reusable "unconfirmed-offer" guardrail | `ENGINE` guideline (content-readiness) |
| 21 | **Image-first full-bleed section w/ overlaid copy** (`DiningPreview`) | edge-to-edge photograph carrying the emotion + a restrained scrim'd copy panel — the archetype for "let the image do the work" sections; distinct from the hero (contained band, not 100vh) | `ENGINE/ui` (Section variant) |
| 22 | **Inline trust chip** (`DiningPreview` certified-halal chip) | a compact bordered/pill trust emblem for use *inside* a copy block (vs the standalone badge #7) — verified-only | `ENGINE/ui` |
| 23 | **Editorial mosaic / gallery showcase** (`GalleryShowcase`) | an "emotional peak" section: asymmetric feature + tall + trio (not a uniform grid), captioned tiles, slow motion-safe hover-zoom, one exploration CTA — reusable premium gallery-teaser | `ENGINE/ui` (Section variant) |
| 24 | **Placeholder-reuse-as-breadth pattern** (`GalleryShowcase` tiles) | a gallery/teaser can compose the already-built labelled placeholder hues (one per vertical) to preview breadth honestly before the shoot — reduces bespoke placeholder work | template pattern |
| — | **Photography-FIRST creative process (client standing rule)** | design order for every section: (1) choose the image → (2) build the composition around it → (3) write copy → (4) place CTAs; images create emotion, copy provides context, CTAs capture intent. Strong candidate for an OS Design-Language principle. | `DESIGN-LANGUAGE` principle |
